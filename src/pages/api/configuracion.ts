...
export async function GET(context: APIContext): Promise<Response> {
    try {
        ...
        const session = await lucia.getSession(context.request);
        ...
        // Renderizar el formulario con los detalles del usuario logueado
        return new Response(renderForm(user), {
...
function renderForm(user: { usuario: { id: string; nombres: string; apellidos: string; user: string; contraseña: string; fecha_creacion: Date; rol_id: number; }; Roles: { id: number; rol_label: string; }; }[]): BodyInit | null | undefined {
